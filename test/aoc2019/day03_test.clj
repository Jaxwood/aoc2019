...
(deftest day03
  ...
  (testing "ab"
    (def expected 30)
    (def actual (day03b ["R8,U5,L5,D3", "U7,R6,D4,L4"]))
    (is (= expected actual)))
  (testing "b"
    (def expected 159)
  ...
  (testing "bb"
    (def expected 610)
    (def actual (day03b ["R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"]))
    (is (= expected actual)))
  (testing "c"
    (def expected 135)
  ...
  (testing "cc"
    (def expected 410)
    (def actual (day03b ["R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"]))
    (is (= expected actual)))
  (testing "parse"
  ...
  (testing "steps a"
    (def expected 20)
    (def actual (steps [3 3] (follow [] [0 0] (map toTuples (parse "R8,U5,L5,D3")))))
    (is (= expected actual)))
  (testing "steps b"
    (def expected 20)
    (def actual (steps [3 3] (follow [] [0 0] (map toTuples (parse "U7,R6,D4,L4")))))
    (is (= expected actual)))
  (testing "day03a"
    (def expected 209)
